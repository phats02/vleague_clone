...
module.exports = {
    ...
    InsertData: async () => {
        ...
        //insert user admin
        const user = await db.any(`select * from "TAIKHOAN" where "TAIKHOAN"."TenTaiKhoanAdmin"='admin'`)
        if (user.length==0) {
            var pwHashed = await bcrypt.hash('admin', saltRounds)
            await db.none(`insert into "TAIKHOAN"("TenTaiKhoanAdmin","MatKhau") values('admin','${pwHashed}')`)
        }
    }
